...
#We essentially have our final model now.
#Ask Liao: After getting the best model from subset regression. Would it be okay after
#adjusting the model, to check for the adj-R^2 to see how better our model is?





#potential prediction and confidence intervals
library(dplyr)
new_data = data %>% select (CarName, fueltype, drivewheel , enginelocation ,
                             wheelbase ,carwidth, enginetype,cylindernumber,enginesize ,
...
new = data.frame(CarName=15, fueltype = 1,drivewheel = 0,
                 enginelocation= 0, wheelbase = 100, carwidth = 7,
                 cylindernumber = 5, enginesize = 250, stroke = 3,
                 compressionratio = 17, horsepower = 18, peakrpm = 4150, enginetype=0)
predict(model4, new,interval = 'prediction', level = 0.95)
#(1.541358 2.22377)
predict(model4, new,interval = 'confidence', level = 0.95)
#(1.548707 2.216421)
#QUESTIONS WE CAN ASK FOR PRESENTATION/Report:
...
#How will the change in enginesize aftect the change in car price after controlling with carname, fueltype, drivewheel, enginelocation,wheeelbase,carwidth,enginetype,cylindernumber,stroke,compressionaration,horsepowe,peakrpm,and the interaction between carname and cylindernumber

# Q3: what is the average height for students whose left arm is 25 cm and left foot is 25 cm? what is the 95%
# confidence interval?
#   
